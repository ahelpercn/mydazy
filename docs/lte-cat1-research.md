...
| 芯片型号 | 系列 | 制程 | 标准 | 核心特性 |
|---------|------|------|------|---------|
| **GX316** | GX31X | **22nm** | LTE Cat.1bis | 五合一（Modem+RF+pSRAM+Flash+PMIC），Star-MC1 应用处理器，小封装，超低功耗 |
| **GX318** | GX30x | 未公开 | LTE Cat.1 | 异构多核，500MHz 独立应用处理器，4MB ROM+4MB pSRAM，XIP 扩展，Data+VoLTE+FOTA 同时支持 |

**GX318 设计赢家**：中移物联（中国移动 IoT 子公司）基于 GX318 推出 **ML307G 系列** Cat.1 模组，采用全国产化芯片及外围器件设计，支持 OpenCPU 开发，适用于智能支付、安防、定位追踪、智慧表计、智能穿戴等场景。

**GX316 适用场景**：功能手机、智能穿戴手表、学生卡、泛物联网通信模组、车载追踪器、智慧安防、智慧支付。

> 注：详细规格（封装尺寸、精确 PSM 电流、具体频段）尚未公开，需联系官方获取 NDA 资料。
